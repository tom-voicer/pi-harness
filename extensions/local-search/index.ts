/**
 * local-search — Self-hosted web search & content extraction engine
 *
 * Replaces the Tavily API backend with a locally-controlled stack:
 *   • Search: DuckDuckGo (duck-duck-scrape) → local SearXNG → public SearXNG
 *   • Extract: fetch → JSDOM + Readability → Turndown (markdown)
 *
 * No API keys, no Docker, no external rate-limits.
 * Set SEARXNG_URL env var to use a local SearXNG instance for maximum reliability.
 */
...
// ===========================================================================
// Backend 1: DuckDuckGo via duck-duck-scrape (primary)
// ===========================================================================
...
function getLocalSearXNGUrl(): string {
  // Check env var every call (not just at load time)
...
async function searxngSearch(baseUrl: string, query: string): Promise<SearchResult[]> {
  ...
  const result = await fetchWithTimeout(url, {
    headers: { Accept: "application/json", "User-Agent": "pi-local-search/1.0" },
    timeout: 10_000,
  });
...
async function extractUrl(
  ...
  try {
    const result = await fetchWithTimeout(url, {
      headers: {
        "User-Agent":
          "Mozilla/5.0 (compatible; pi-local-search/1.0)",
          ...
          content: [
            {
              ...
              text:
                ...
                `See: https://github.com/luxiaolei/searxng-crawl4ai-mcp`,
            },
          ],
