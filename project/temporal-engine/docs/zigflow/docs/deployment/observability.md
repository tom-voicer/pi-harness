# Observability | Zigflow

> Source: [https://zigflow.dev/docs/deployment/observability](https://zigflow.dev/docs/deployment/observability)
...
Liveness: returns `200 OK` when the process is running

`GET /readyz`

Readiness: returns `200 OK` when the worker is connected and polling
...
```
healthcheck:  test: ["CMD-SHELL", "curl -f http://localhost:3000/readyz || exit 1"]  interval: 10s  timeout: 5s  retries: 3
```
...
### CloudEvents metrics[​](#cloudevents-metrics "Direct link to CloudEvents metrics")
...
`zigflow_events_emitted_total`

`client`, `type`

Total events emitted per client and event type

`zigflow_events_undelivered_total`

`client`, `type`

Events that failed to deliver

`zigflow_events_emit_duration_seconds`
...
### Scraping in Kubernetes[​](#scraping-in-kubernetes "Direct link to Scraping in Kubernetes")
...
## CloudEvents[​](#cloudevents "Direct link to CloudEvents")

tip

For full configuration options, event structure, file output examples and debugging guidance, see [Debugging workflows](https://zigflow.dev/docs/dsl/debugging).

Zigflow can emit [CloudEvents v1.0](https://cloudevents.io/) at key points during workflow execution. This is the primary mechanism for real-time observability into running workflows.
...
```
clients:  - name: file-logger    protocol: file    target: ./tmp/events  - name: http-sink    protocol: http    target: "{{ .env.HTTP_EVENTS_URL }}"    options:      timeout: 5s      method: POST
```
...
Events written as YAML, one file per workflow execution
...
`dev.zigflow.workflow.started`

A workflow execution begins

`dev.zigflow.workflow.completed`

A workflow execution completes successfully

`dev.zigflow.task.started`

A task begins

`dev.zigflow.task.retried`

A task is retried after failure

`dev.zigflow.task.cancelled`

A task is cancelled

`dev.zigflow.task.faulted`

A task fails

`dev.zigflow.task.completed`

A task completes successfully

`dev.zigflow.iteration.completed`

A task iteration completes
...
*   [Deploying overview](https://zigflow.dev/docs/deployment/intro): runtime ports and configuration
*   [Docker](https://zigflow.dev/docs/deployment/docker): Docker and Compose configuration
*   [Kubernetes](https://zigflow.dev/docs/deployment/kubernetes): Helm chart deployment
*   [Debugging workflows](https://zigflow.dev/docs/dsl/debugging): CloudEvents in detail