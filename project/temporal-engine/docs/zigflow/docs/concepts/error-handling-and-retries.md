# Error Handling and Retries | Zigflow

> Source: [https://zigflow.dev/docs/concepts/error-handling-and-retries](https://zigflow.dev/docs/concepts/error-handling-and-retries)
...
## Activity retries[​](#activity-retries "Direct link to Activity retries")
...
## Configuring retries[​](#configuring-retries "Direct link to Configuring retries")
...
```
- callApi:    metadata:      activityOptions:        retryPolicy:          initialInterval:            seconds: 5          backoffCoefficient: 1.5          maximumInterval:            seconds: 30          maximumAttempts: 3          nonRetryableErrorTypes:            - validation-error    call: http    with:      method: post      endpoint: https://api.example.com/process
```
...
## The try/catch pattern[​](#the-trycatch-pattern "Direct link to The try/catch pattern")
...
```
- fetchUser:    try:      - getUser:          call: http          output:            as:              user: ${ . }          with:            method: get            endpoint: https://api.example.com/users/999    catch:      do:        - handleMissing:            output:              as:                error: ${ . }            set:              message: User not found
```
...
## The raise task[​](#the-raise-task "Direct link to The raise task")
...
```
- checkPermission:    switch:      - denied:          when: ${ $data.role != "admin" }          then: rejectRequest      - default:          then: processRequest- rejectRequest:    raise:      error:        type: https://serverlessworkflow.io/spec/1.0.0/errors/authorization        status: 403        title: Forbidden        detail: Only admin users can perform this action
```
...
**Not setting `startToCloseTimeout` for long-running activities.** The default start-to-close timeout is 15 seconds. Long-running activities (such as container executions or waiting on external systems) should increase this via `metadata.activityOptions.startToCloseTimeout`.

* * *

*   [Try task](https://zigflow.dev/docs/dsl/tasks/try): full reference
*   [Raise task](https://zigflow.dev/docs/dsl/tasks/raise): full reference
*   [Activity options](https://zigflow.dev/docs/dsl/metadata/activity-options): retry policy reference
*   [How Zigflow runs](https://zigflow.dev/docs/concepts/how-zigflow-runs): execution model